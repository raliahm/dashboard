...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (req.method === 'PATCH') {
      ...
      await db.execute({
        sql: 'UPDATE assignments SET title = ?, type = ?, description = ?, due_date = ?, class_name = ?, status = ?, priority = ? WHERE id = ? AND user_id = ?',
        args: [title, type, description, due_date, class_name, status, priority, id, userId],
      });
    ...
    if (req.method === 'DELETE') {
      await db.execute({
        sql: 'DELETE FROM assignments WHERE id = ? AND user_id = ?',
        args: [id, userId],
      });
    ...
    console.error('Assignments [id] API error:', error);
