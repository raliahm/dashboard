// PATCH/DELETE for assignments_tracker by id
import { createClient } from '@libsql/client';
const db = createClient({
  url: process.env.TURSO_DB_URL,
  authToken: process.env.TURSO_DB_TOKEN,
});
import { verifyGoogleToken } from '../../api/auth.js';

export default async function handler(req, res) {
  console.log('--- Assignment Tracker [id] handler called ---');
  try {
    const { id } = req.query;
    console.log('Request method:', req.method);
    console.log('Assignment id:', id);
    const auth = req.headers.authorization;
    if (!auth) {
      console.log('No auth header');
      return res.status(401).json({ error: 'Missing token' });
    }
    const user = await verifyGoogleToken(auth.split(' ')[1]);
    if (!user) {
      console.log('Invalid token');
      return res.status(401).json({ error: 'Invalid token' });
    }
    const user_id = user.sub;
    console.log('User ID:', user_id);

    if (req.method === 'PATCH') {
      console.log('PATCH method');
      let body = req.body;
      if (typeof body === 'string') {
        try { body = JSON.parse(body); } catch (e) { console.log('Body parse error', e); body = {}; }
      }
      const { name, due_date } = body;
      console.log('PATCH body:', body);
      if (!name || !due_date) {
        console.log('Missing fields');
        return res.status(400).json({ error: 'Missing fields' });
      }
      const d = new Date(due_date);
      const formatted_due = `${String(d.getMonth()+1).padStart(2,'0')}-${String(d.getDate()).padStart(2,'0')}-${d.getFullYear()}`;
      console.log('Updating assignment:', { name, due_date, formatted_due, id, user_id });
      const result = await db.execute(
        'UPDATE assignments_tracker SET name=?, due_date=?, formatted_due=? WHERE id=? AND user_id=?',
        [name, due_date, formatted_due, id, user_id]
      );
      console.log('Update result:', result);
      return res.json({ id, name, due_date, formatted_due, user_id });
    } else if (req.method === 'DELETE') {
      console.log('DELETE method');
      const result = await db.execute('DELETE FROM assignments_tracker WHERE id=? AND user_id=?', [id, user_id]);
      console.log('Delete result:', result);
      return res.json({ success: true });
    } else {
      console.log('Method not allowed:', req.method);
      res.status(405).json({ error: 'Method not allowed' });
    }
  } catch (err) {
    console.log('Assignment Tracker [id] error:', err);
    res.status(500).json({ error: 'Internal server error', details: err.message });
  }
}
